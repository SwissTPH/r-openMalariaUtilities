...
##' @param subpop Either NULL or string, concatenation component+"-"+subpop will be id in subpopulation intervention should be restricted to (see
...
## Furthermore, if subpop is not NULL, deployment to subpopulation with restricToSubPop id
      ...
      if (cumulative == TRUE) {
        temp <- append(temp, list(
          cumulativeCoverage = list(
            component = paste0(component, "_", subpop)
          )
        ))
      }
      if (!is.null(subpop)) {
