...
If survey outputs should be disaggregated by intervention cohort membership,
add a `cohorts` entry with `monitoringCohortsGen()`. This function also writes
the cohort IDs to the experiment cache so `createBaseXml()` can create the
`thirdDimension` lookup table used when reading OpenMalaria output files.

```{r monitoring-cohorts, eval=FALSE}
baseList[["monitoring"]] <- c(
  baseList[["monitoring"]],
  list(
    cohorts = monitoringCohortsGen(c("LLINusers", "SMCusers"))
  )
)
```
...
For cohort deployment, first define a recruitment-only component and then pass
the same component ID through the `cohort` argument when deploying the
intervention. In the generated XML, recipients receive both the intervention
component and the cohort component in the same deployment block.

```{r intervention-cohorts, eval=FALSE}
baseList <- defineCohort(baseList, ids = c("LLINusers", "SMCusers"))
...
Postprocessing functions which aggregate translated output should split
`third_dimension` values such as `LLINusers:0-1` into cohort and age-group
columns before aggregating by cohort-specific age groups.
